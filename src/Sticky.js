...
var getBorder = (props) =>
  `${
    props.borderWidth === undefined ? "2px" : props.borderWidth || "0px"
  } solid ${props.borderColor || "#e5e5e5"}`;
...
const Wrapper = styled("div").attrs(() => ({
  ...
  &
    ${Row}:not(:nth-last-child(-n+${(props) =>
      (props.stickyFooterCount || 0) + 1}))
    ${Cell} {
  ...
  & ${Row}:nth-child(${(props) => `-n+${props.stickyHeaderCount}`}) ${Cell} {
    ...
    background-color: ${(props) => props.headerColor || "lightgrey"};
  }
  ...
  & ${Row} ${Cell}:nth-child(-n+${(props) => props.leftStickyColumnCount}) {
    ...
    background-color: ${(props) => props.leftStickyColumnsColor || "lightgrey"};
  }
  &
    ${Row}
  ...
  ${(props) => {
    ...
    for (i = 0; i < insets.header.length; i++) {
      styles += `& ${Row}:nth-child(${i + 1}) ${Cell} { top: ${
        insets.header[i]
      }px; }`;
    }

    for (i = 0; i < insets.footer.length; i++) {
      styles += `& ${Row}:nth-last-child(${i + 1}) ${Cell} { bottom: ${
        insets.footer[i]
      }px; }`;
    }

    for (i = 0; i < insets.leftColumn.length; i++) {
      styles += `& ${Row} ${Cell}:nth-child(${i + 1}) { left: ${
        insets.leftColumn[i]
      }px; }`;
    }

    for (i = 0; i < insets.rightColumn.length; i++) {
      styles += `& ${Row} ${Cell}:nth-last-child(${i + 1}) { right: ${
        insets.rightColumn[i]
      }px; }`;
    }
  ...
  & ${Row}:nth-child(-n+${(props) =>
    ...
    props.leftStickyColumnCount}) {
    z-index: ${(props) =>
      Math.max(props.headerZ || 2, props.leftColumnZ || 2) + 1};
  }
  &
    ${Row}:nth-child(-n+${(props) => props.stickyHeaderCount})
    ${Cell}:nth-last-child(-n+${(props) => props.rightStickyColumnCount}) {
    z-index: ${(props) =>
      Math.max(props.headerZ || 2, props.rightColumnZ || 2) + 1};
  }
  &
    ${Row}:nth-last-child(-n+${(props) => props.stickyFooterCount})
    ${Cell}:nth-child(-n+${(props) => props.leftStickyColumnCount}) {
    z-index: ${(props) =>
      Math.max(props.footerZ || 2, props.leftColumnZ || 2) + 1};
  }
  &
    ${Row}:nth-last-child(-n+${(props) => props.stickyFooterCount})
    ${Cell}:nth-last-child(-n+${(props) => props.rightStickyColumnCount}) {
    z-index: ${(props) =>
      Math.max(props.footerZ || 2, props.rightColumnZ || 2) + 1};
  }
`;
...
class StickyTable extends React.Component {
  ...
  checkForStickySizeChanges() {
    ...
    if (
      JSON.stringify(stickyInsets) !== JSON.stringify(this.state.stickyInsets)
    ) {
